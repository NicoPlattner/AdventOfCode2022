...
function main() {
	...
	const rootSize = getFolderSize(fileSystem['/']);
	const required = 30000000 - (70000000 - rootSize);

	console.log(folders.map(folder => getFolderSize(folder)).sort((a, b) => a - b).filter(size => size > required)[0]);
}
